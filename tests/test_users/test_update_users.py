...
async def test_update_user(test_data, client: AsyncClient, user_test_samples, get_user, test_auth_headers):
    ...
    resp = await client.patch(f"/user/{test_user_id}", json=test_data, headers=test_auth_headers)
...
async def test_no_body_create(client: AsyncClient, user_test_samples, test_auth_headers):
    test_user_id = 1
    resp = await client.patch(f"/user/{test_user_id}", headers=test_auth_headers)
